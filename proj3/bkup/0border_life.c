    ...
    if (myRank == (numProcs - 1)) {
        ...
        for (i = 0; i < numProcs; ++i) {
            ...
            for (r = 0; r < *numRows; ++r) {
                ...
                // If this is the first or last row in the matrix,
                // add a top and bottom border of 0's
                if ((i == 0 && r == 0) || (i == numProcs-1 && r == *numRows-1)){
                    for (c = 0; c < *numCols; ++c) {
                        (*subMatrix)[r][c] = '0';
                    }

                // Else read in data for row from file
                } else {

                    for (c = 1; c < *numCols-1; ++c) {
                        // Abort if file read fails
                        if (fscanf(matrixFile, "%c", (*subMatrix)[r] + c) != 1) {
                            MPI_ABORT(MPI_COMM_WORLD, OPEN_FILE_ERROR);
                        }
                    }

                    // Add 0's to left and right end of row
                    (*subMatrix)[r][0] = '0';
                    (*subMatrix)[r][c] = '0';
            ...
            // I don't need to send my data to myself
            if (i != myRank) {
                MPI_Send(*subMatrix, size * (*numCols), MPI_CHAR, i, 
        ...
        // Receive matrix data
        MPI_Recv(*bulkStorage, myRows * (*numCols), MPI_CHAR, numProcs - 1,
