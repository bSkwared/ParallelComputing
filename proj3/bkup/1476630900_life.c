    ...
    if (myRank == (numProcs - 1)) {
        ...
        for (i = 0; i < numProcs; ++i) {
            ...
            for (r = 0; r < size; ++r) {
                ...
                // If this is the top of the bottom, add a row of 0's
                if ((i == 0 && r == 0) || (i == numProcs - 1 && r == size-1)) {
                    for (c = 0; c < *numCols; ++c) {
                        (*bulkStorage)[c] = '0';
                    }

                // Else, read in the row and put 0's at front and back
                } else {
                    ...
                    bytesRead = fread((*subMatrix)[r] + 1, sizeof(char),
                                        *numCols - 2, matrixFile);

                    (*subMatrix)[r][0] = '0';
                    (*subMatrix)[r][*numCols - 1] = '0';
                }
            }

            if (i != myRank) {
                MPI_Send(*bulkStorage, size * (*numCols), MPI_CHAR, i, 
