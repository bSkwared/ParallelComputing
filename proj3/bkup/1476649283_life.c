...
int main(int argc, char* argv[]) {
   ...
   if (myRank > 0) {
        MPI_Send(matrix[1], myCols, MPI_CHAR, myRank - 1, 
                    DATA_MSG, MPI_COMM_WORLD);
   }

   if (myRank < numProcs - 1) {
        MPI_Recv(matrix[myRows - 1], myCols, MPI_CHAR, myRank + 1,
                    DATA_MSG, MPI_COMM_WORLD, &status);

        MPI_Send(matrix[myRows - 2], myCols, MPI_CHAR, myRank + 1, 
                    DATA_MSG, MPI_COMM_WORLD);
   }

   if (myRank > 0) {
        MPI_Recv(matrix[0], myCols, MPI_CHAR, myRank - 1,
                    DATA_MSG, MPI_COMM_WORLD, &status);
   }
sleep(myRank);    
    printf("\n\nmyRank : %d\n", myRank);
